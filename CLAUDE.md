...
**All KDE URLs MUST:** 1. Use `https://www.education.ky.gov/` (with
`www.` prefix) — returns 403 without it 2. Include a browser-like
...
3.  [Student Enrollment
    Page](https://www.education.ky.gov/districts/enrol/Pages/default.aspx) -
    Main enrollment data landing

### Known Issues Fixed

1.  **403 Forbidden** - Fixed by adding `www.` prefix to all URLs
2.  **403 with User-Agent** - Fixed by adding browser-like User-Agent
    header to httr requests
...
warnings - \[ \] `pytest tests/test_pykyschooldata.py` — all tests
pass - \[ \]
...
The `type` column has values `State`, `District`, and `School`. The
